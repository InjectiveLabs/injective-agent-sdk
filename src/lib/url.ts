...
export function assertPublicUrl(raw: string, label = "URL"): void {
  ...
  if (BLOCKED_PROTOCOLS.has(parsed.protocol)) {
    throw new CliError(`${label} scheme "${parsed.protocol}" is not allowed. Use https:// or http://.`);
  }
  const host = parsed.hostname.toLowerCase();
  if (host === "localhost" || PRIVATE_HOSTNAME_PATTERNS.some(r => r.test(host))) {
    throw new CliError(`${label} points to a private/internal address and is not allowed.`);
  }
}
